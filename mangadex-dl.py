...
# TODO - pull current chapter of current manga being dled from mal manga list
...
def dl(manga_idList, lang_code, zip_up, ds, outdir, download):
    mangaListData = []
    for manga_id in manga_idList:
        uuid = manga_id
        ...
        if download is False:
            ...
            dl_list = " " + ', '.join(map(
                ...
                chap_list))
        mangaListData.append([title, chap_list, requested_chapters, dl_list, outdir, ds, zip_up])
    for mangaData in mangaListData:
        downloadRequested(mangaData[0], mangaData[1], mangaData[2], mangaData[3], mangaData[4], mangaData[5], mangaData[6])


def downloadRequested(title, chap_list, requested_chapters, dl_list, outdir, ds, zip_up):
    ...
    for index, chapter in enumerate(requested_chapters):
        if chapter['attributes']['chapter'] in chaptersDownloaded:  # repeating chapter fix
            continue
        chaptersDownloaded.append(chapter['attributes']['chapter'])
    ...
    try:
        imageToPDF.main(title, outfile)  # TODO - call to pdf routine
    except:
        print('PDFFing failed. ')
    print('Done')



def emptyImageFile(path):
    for f in os.listdir(path):
        os.remove(os.path.join(path, f))


def clearFolder():
    folder = r'C:\Users\User\PycharmProjects\projects\mangaScraper\mangadex-dl\download'  # TODO - change to be dynamic
...
if __name__ == "__main__":
    ...
    url = ""
    while url != '1':
        url = input("Enter manga URL or ID ('1' to quit adding links): ").strip()
        urlList.append(url)

    downloadAll = False
    # choice = input('Download all chapters? (Y/N)')
    # if choice == 'Y':
    #     print('Downloading all')
    #     downloadAll = True

    manga_idList = []
    for url in urlList[:-1]:  # avoids 'STOP' last keyword
        try:
            url_parts = url.split('/')
            manga_id = find_id_in_url(url_parts)
            manga_idList.append(manga_id)
        except:
            print("Error with URL.")
            exit(1)
        # dl(manga_id, lang_code, args.cbz, args.datasaver, args.outdir, downloadAll)

    dl(manga_idList, lang_code, args.cbz, args.datasaver, args.outdir, downloadAll)
